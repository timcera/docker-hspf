      MODULE SCENMOD
        ...
        INTEGER(C_INTPTR_T),      SAVE    :: HPIN = 0
        INTEGER(C_INTPTR_T),      SAVE    :: HPOUT = 0
      ...
      CONTAINS
        ...
        SUBROUTINE M_SPIPH (HIN,HOUT)
          USE WIN32
          USE ISO_C_BINDING
          INTEGER(C_INTPTR_T),         INTENT(IN) :: HIN, HOUT
          CHARACTER*20 S
...
2000      FORMAT ("In M_SPIPH",2I5)
        ...
        INTEGER FUNCTION UPDATESTATUS(I,J)
          ...
          integer(C_LONG) P
          integer(C_LONG) K
          type(OVERLAPPED_T), pointer :: lpOverlapped
          CHARACTER(1) CNULL
          INTEGER(C_INT32_T) NOTUSED

          INTEGER(C_LONG) lr, la, lm
          ...
          ELSE !just looking for Pause or Cancel
            IF (HPIN .GT. 0) THEN ! check for user messages
              L = PeekNamedPipe(HPIN, CNULL, &
                            NOTUSED, lr, la, lm)
              IF (L .NE. 0 .AND. LA .GT. 0) THEN ! a message
                L = ReadFile(HPIN, MMAP, la, lr, &
                             lpOverlapped)
                IF (DBGLEV>0) THEN
...
10        CONTINUE
        ...
        SUBROUTINE M_GLOBLK (SDATE,EDATE,OUTLEV, &
          ...
          CALL GLOBLK (I0,MSG%FUN,I0, &
                       ECOUNT, &
                       SEDAT,SDATIM,EDATIM,RUNMIN,OUTLEV,SPOUT, &
                       RESMFG,RUNFG,EMFG,RNINFO,IHMFG)
          IF (DBGLEV > 0) THEN
        ...
        INTEGER,INTENT(OUT) :: LNFLDS,LSCOL(30),LFLEN(30), &
                               ...
                               LIDEF(30),LNMHDR,RETCOD,ISECT,IREPT
 ...
 10       CONTINUE
        ...
        SUBROUTINE SYNC_TIME (DELT,OPST,OPND,OPNTAB,EXUPFG,EXTWID)
          ...
          INTEGER                      :: BINADD,BINU,LFIL,CLEN1,CLEN2,I
          INTEGER                      :: VARADD(4),RECORD,OFFSET,CLEN3
          INTEGER                      :: VLEN(4),IV,NVLIB,MXSUB(4),IX
          INTEGER                      :: ADDR,VARTYP(4),IVAL(4,1),J
          REAL                         :: RVAL(4,12)
