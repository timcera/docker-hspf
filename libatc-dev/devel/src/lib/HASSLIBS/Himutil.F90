...
!GCC$ ATTRIBUTES DLLEXPORT :: CloseFile
...
!GCC$ ATTRIBUTES DLLEXPORT :: Read_MODFLOW_Unformatted
...
829   continue
      ...
      if(INFTYPE .eq. 0 .and. (.not. fileopened)) then  ! input file is binary
         !  WRITE(*,*)'Opening Input Binary File: ',INFNAME
           OPEN(UNIT=INFUNIT,FILE=INFNAME,STATUS='OLD',ERR=1929)
      else if(INFTYPE .eq. 1 .and. (.not. fileopened)) then   ! input file is unformatted
!              WRITE(*,*)'Opening Input Unformatted File: ',INFNAME
              OPEN(UNIT=INFUNIT,FILE=INFNAME,STATUS='OLD',ERR=1929,ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
      else
...
!GCC$ ATTRIBUTES DLLEXPORT :: Read_MODFLOW_3D_Unformatted 
...
!GCC$ ATTRIBUTES DLLEXPORT :: Read_MODFLOW_List 
...
829   continue
   ...
   !        WRITE(*,*)'Opening Output Binary File: ',OUTFNAME
           OPEN(UNIT=OUTFUNIT,FILE=OUTFNAME,STATUS='REPLACE',ACTION='WRITE')
...
!GCC$ ATTRIBUTES DLLEXPORT :: Read_Integer_Array 
...
! Check if input file is open
      ...
      IF(.NOT. OPD) THEN
         IF(infUnit .gt. 0) OPEN(UNIT=IABS(infUnit),FILE=infName,STATUS='OLD',&
         ERR=469,ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='READ')
         IF(infUnit .lt. 0) OPEN(UNIT=IABS(infUnit),FILE=infName,STATUS='OLD', &
         ERR=469,ACTION='READ')
      ENDIF
...
!--CHECK IF THE EXTERNAL FILE HAS BEEN OPENED.  IF NOT, OPEN
  250 INQUIRE(UNIT=IABS(IREAD), OPENED = OPD)
      IF(.NOT. OPD) THEN
         ...
         ENDIF
         IF(IREAD .lt. 0) then
             OPEN(UNIT=IABS(IREAD),FILE=infName,STATUS='OLD',ERR=469,ACTION='READ')
             READ(IABS(IREAD))
             READ(IABS(IREAD)) IA
         ENDIF
      ENDIF
...
!GCC$ ATTRIBUTES DLLEXPORT :: Read_Real_Array
...
!GCC$ ATTRIBUTES DLLEXPORT :: Write_Integer_Array
...
! Check if output file is open
      ...
      IF(.NOT. OPD) THEN
         IF(outfUnit .gt. 0 .and. outfType .eq. 0) then
             OPEN(UNIT=outfUnit,FILE=outfName,ERR=469, &
                  ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='WRITE')
         else IF(outfUnit .gt. 0 .and. outfType .eq. 1) then
             OPEN(UNIT=outfUnit,FILE=outfName,STATUS='REPLACE',ERR=469, &
                  ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='WRITE')
         else IF(outfUnit .lt. 0 .and. outfType .eq. 0) then
             OPEN(UNIT=-outfUnit,FILE=outfName,ERR=469,ACTION='WRITE')
         else IF(outfUnit .lt. 0 .and. outfType .eq. 1) then
             OPEN(UNIT=-outfUnit,FILE=outfName,STATUS='REPLACE',ERR=469,ACTION='WRITE')
         ENDIF
      ENDIF
...
!GCC$ ATTRIBUTES DLLEXPORT :: Write_Real_Array
...
! Check if output file is open
      ...
      IF(.NOT. OPD) THEN
         IF(outfUnit .gt. 0 .and. outfType .eq. 0) then
             OPEN(UNIT=outfUnit,FILE=outfName,ERR=469,ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='WRITE')
         else IF(outfUnit .gt. 0 .and. outfType .eq. 1) then
...
!GCC$ ATTRIBUTES DLLEXPORT :: Write_Binout
!      DLL_EXPORT Write_Binout
 ...
 2000 FORMAT (6A1,'  ')
      ...
      IF (OpenFlag .eq. 1) THEN
        IF (FirstLoop .eq. 1) THEN
          ...
          OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED', &
 ...
 10       CONTINUE
            OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED', &
                  STATUS='OLD',POSITION='APPEND',IOSTAT=IOS,ERR=20)
 20         CONTINUE
            I =I+ 1
